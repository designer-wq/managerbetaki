import{o as a}from"./index-0gnM_rEh.js";const g=async r=>{const t=a();if(t)try{await t.from("logs").insert([r])}catch(e){console.error("Failed to log action:",e)}},_=async r=>{const t=a();if(!t)return[];let{data:e,error:n}=await t.from(r).select("*").order("created_at",{ascending:!1});if(n){const o=await t.from(r).select("*");return o.error?(console.error(`Error fetching ${r}:`,o.error),[]):o.data||[]}return e||[]},h=async()=>{const r=a();if(!r)return[];const{data:t,error:e}=await r.from("demands").select(`
            *,
            origins ( name ),
            demand_types ( name ),
