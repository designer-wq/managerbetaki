...
const Sidebar = () => {
  ...
  const menuItems = [
    { icon: LayoutDashboard, label: 'Dashboard', path: '/', resource: 'dashboard' },
    { icon: ListTodo, label: 'Demandas', path: '/demands', resource: 'demands' },
    { icon: Users, label: 'Equipe', path: '/users', resource: 'team' },
    { icon: BarChart3, label: 'Relatórios', path: '/reports', resource: 'reports' },
    { icon: Target, label: 'Metas', path: '/goals', resource: 'goals' },
    { icon: History, label: 'Histórico', path: '/activity-log', resource: 'activity_log' },
  ...
  return (
    <aside className={`hidden md:flex flex-col h-full border-r border-border-dark bg-surface-dark p-4 justify-between shrink-0 transition-all duration-300 ${isCollapsed ? 'w-20' : 'w-64'}`}>
