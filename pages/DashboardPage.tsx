...
const DashboardPage = () => {
  ...
  const getStatusColor = (status: string) => {
    switch (status?.toLowerCase()) {
      case 'concluído': return 'text-primary bg-primary/10 border-primary/20';
      case 'em progresso': return 'text-yellow-500 bg-yellow-500/10 border-yellow-500/20';
      case 'atrasado': return 'text-red-500 bg-red-500/10 border-red-500/20';
      case 'revisão': return 'text-blue-500 bg-blue-500/10 border-blue-500/20';
        ...
        <div className="max-w-7xl mx-auto flex flex-col gap-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="card-enterprise card-primary flex flex-col justify-between gap-4 p-6 relative overflow-hidden group">
              ...
              <div className="flex items-start justify-between relative z-10">
                <div>
                  <p className="text-white text-contrast-title text-sm font-medium">Demandas Totais</p>
                  <h3 className="text-white text-4xl font-bold mt-2 text-contrast-value">{stats.total}</h3>
                </div>
              ...
              <div className="flex items-center gap-2 relative z-10">
                <span className="text-primary bg-primary/10 px-2 py-0.5 rounded text-xs font-bold">Atualizado</span>
                <span className="text-zinc-500 text-contrast-sub text-xs">agora mesmo</span>
              </div>
            </div>
            ...
            <div className="card-enterprise card-secondary flex flex-col justify-between gap-4 p-6 relative overflow-hidden group">
              ...
              <div className="flex items-start justify-between relative z-10">
                <div>
                  <p className="text-zinc-300 text-contrast-title text-sm font-medium">Ações Urgentes</p>
                  <h3 className="text-orange-400 text-4xl font-bold mt-2 text-contrast-value">{stats.urgent}</h3>
                </div>
              ...
              <div className="flex items-center gap-2 relative z-10">
                <span className="text-orange-400 bg-orange-400/10 px-2 py-0.5 rounded text-xs font-bold">Atenção</span>
                <span className="text-zinc-500 text-contrast-sub text-xs">requer ação imediata</span>
              </div>
            </div>
            ...
            <div className="card-enterprise card-secondary flex flex-col justify-between gap-4 p-6 relative overflow-hidden group">
              ...
              <div className="flex items-start justify-between relative z-10">
                <div>
                  <p className="text-zinc-300 text-contrast-title text-sm font-medium">Carga de Trabalho</p>
                  <h3 className="text-blue-400 text-4xl font-bold mt-2 text-contrast-value">{stats.workload}%</h3>
                </div>
              ...
              <div className="w-full bg-zinc-800 h-1.5 rounded-full mt-1 relative z-10">
                <div className="bg-blue-400 h-1.5 rounded-full" style={{ width: `${stats.workload}%` }}></div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 card-enterprise p-6 flex flex-col">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h3 className="text-primary text-lg font-bold">Volume Semanal</h3>
                  <p className="text-zinc-400 text-sm">Distribuição por Dia da Semana</p>
                </div>
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between px-2">
              <h3 className="text-primary text-xl font-bold tracking-tight">Demandas Recentes</h3>
              <button className="text-sm text-zinc-400 hover:text-primary transition-colors font-medium">Ver Todas</button>
            </div>
                  ...
                  <table className="w-full text-left border-collapse table-auto">
                    <thead>
                      <tr className="border-b border-zinc-800 bg-zinc-950">
                        <th className="p-4 text-xs font-semibold uppercase tracking-wider text-primary">Título</th>
                        <th className="p-4 text-xs font-semibold uppercase tracking-wider text-primary">Responsável</th>
                        <th className="p-4 text-xs font-semibold uppercase tracking-wider text-primary">Status</th>
                        <th className="p-4 text-xs font-semibold uppercase tracking-wider whitespace-nowrap text-primary">Prazo</th>
                        <th className="p-4 text-xs font-semibold uppercase tracking-wider text-primary">Tipo</th>
                        <th className="p-4 text-xs font-semibold uppercase tracking-wider text-primary">Origem</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-zinc-800">
                      {demands.slice(0, 5).map((demand) => (
                        <tr key={demand.id} className="group hover:bg-zinc-800/50 transition-colors">
                          <td className="p-4">
                            <div className="flex flex-col">
                              <span className="text-white font-medium">{demand.title}</span>
                              <span className="text-xs text-zinc-500">#{demand.id.slice(0, 6)}</span>
                            </div>
                          </td>
                          <td className="p-4">
                          ...
                          <td className="p-4">
                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${getStatusColor(demand.statuses?.name)}`}>
                          ...
                          <td className="p-4 text-sm text-zinc-400">{demand.origins?.name || '-'}</td>
                        </tr>
                      ))}
                      {demands.length === 0 && (
                        <tr><td colSpan={6} className="p-8 text-center text-zinc-500">Nenhuma demanda recente encontrada.</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
