...
import { Filter, Download, Plus, Clock, AlertCircle, CheckCircle, MoreVertical, FileDown, Search, Copy, Trash2, Flag, Pencil, ArrowUpRight, Users, Calendar, ChevronDown, ArrowUpDown } from 'lucide-react';
...
import BulkActionsBar from '../components/demands/BulkActionsBar';
...
const DemandsPage = () => {
   ...
   // Bulk Actions Handlers (must be after loadDemands is defined)
   ...
   }, [sortedDemands.length, activeTab, searchTerm, selectedDesigner, dateFilter]);

   // Get paginated slice of sorted demands
   const paginatedDemands = sortedDemands.slice(
               ...
               <div className="mb-6 flex flex-col md:flex-row gap-4">
                  ...
                  <SavedFiltersMenu
                     ...
                     onApplyFilter={(filters) => {
                        ...
                        if (filters.selectedDesigner !== undefined) setSelectedDesigner(filters.selectedDesigner);
                        ...
                        <thead>
                           <tr className="border-b border-zinc-800 bg-zinc-900/50 text-contrast-sub">
                              <th className="px-2 py-3 w-[3%]">
                                 <input
                                    type="checkbox"
                                    checked={bulkSelection.isAllSelected && paginatedDemands.length > 0}
                                    onChange={(e) => {
                                       e.stopPropagation();
                              ...
                              <th className="px-2 py-3 font-medium text-zinc-400 text-[10px] uppercase tracking-wider whitespace-nowrap w-[8%]">Tempo</th>
                              <th
                        ...
                        <tbody className="divide-y divide-zinc-800/50">
                           ...
                           {paginatedDemands.map((item) => (
                              <tr
                                 ...
                                 className={`group transition-colors relative cursor-pointer hover:bg-zinc-800/30 ${bulkSelection.isSelected(item.id) ? 'bg-primary/5' : ''}`}
                              >
