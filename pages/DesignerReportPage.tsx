        ...
        <div className="flex-1 flex flex-col h-full overflow-hidden bg-zinc-950">
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-8 pb-20 custom-scrollbar">
                <div className="max-w-7xl mx-auto">
                    ...
                    {/* Summary Table */}
                    <div className="mt-8 bg-zinc-900/50 border border-zinc-700/50 rounded-2xl overflow-hidden">
                        <div className="p-4 border-b border-zinc-700/50">
                            <h3 className="font-semibold text-white">Resumo Detalhado</h3>
                        </div>
                        <div className="overflow-x-auto">
                            <table className="w-full">
                                <thead>
                                    <tr className="border-b border-zinc-700/50">
                                        <th className="text-left py-3 px-4 text-xs font-medium text-zinc-500 uppercase">Designer</th>
                                        <th className="text-center py-3 px-4 text-xs font-medium text-zinc-500 uppercase">Total</th>
                                        <th className="text-center py-3 px-4 text-xs font-medium text-zinc-500 uppercase">Concluídas</th>
                                        <th className="text-center py-3 px-4 text-xs font-medium text-zinc-500 uppercase">No Prazo</th>
                                        <th className="text-center py-3 px-4 text-xs font-medium text-zinc-500 uppercase">Atrasadas</th>
                                        <th className="text-center py-3 px-4 text-xs font-medium text-zinc-500 uppercase">A Fazer</th>
                                        <th className="text-center py-3 px-4 text-xs font-medium text-zinc-500 uppercase">Vencidas</th>
                                        <th className="text-center py-3 px-4 text-xs font-medium text-zinc-500 uppercase">Eficiência</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {filteredDesignerStats.map((designer) => (
                                        <tr key={designer.id} className="border-b border-zinc-800/50 hover:bg-zinc-800/30 transition-colors">
                                            <td className="py-3 px-4">
                                                <div className="flex items-center gap-3">
                                                    {designer.avatar_url ? (
                                                        <img src={designer.avatar_url} alt={designer.name} className="h-8 w-8 rounded-full object-cover" />
                                                    ) : (
                                            ...
                                            <td className="text-center py-3 px-4 text-zinc-300">{designer.totalDeadlineThisPeriod}</td>
                                            <td className="text-center py-3 px-4 text-emerald-400 font-medium">{designer.completed}</td>
                                            <td className="text-center py-3 px-4 text-green-400">{designer.completedOnTime}</td>
                                            <td className="text-center py-3 px-4 text-amber-400">{designer.completedLate}</td>
                                            <td className="text-center py-3 px-4 text-blue-400">{designer.inProduction + designer.pending}</td>
                                            <td className="text-center py-3 px-4 text-red-400">{designer.overdue}</td>
                                            <td className="text-center py-3 px-4">
                                                <span className={`px-2 py-1 rounded-full text-xs font-bold ${getEfficiencyBg(designer.efficiency)} ${getEfficiencyColor(designer.efficiency)}`}>
                                                    {designer.efficiency}%
                                                </span>
                                            </td>
                                        </tr>
